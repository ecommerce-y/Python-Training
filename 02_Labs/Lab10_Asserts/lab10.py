...
def time_to_military(s):
    ...
    assert is_time_format(s), "labubububububu"
...
def is_time_format(s):
    """
    Returns: True if s is a string in 12-hour format <hours>:<min> AM/PM
    ...
    if not isinstance(s, str):
        labubu = False
    elif s[0:2].isnumeric() and int(s[0:2])<13:
        labubu = s[2] == ':' and s[3:5].isnumeric() and s[5] == ' ' and (s[6:8] == 'AM' or s[6:8] == 'PM') and len(s) == 8
    elif s[0].isnumeric():
        labubu = s[1] == ':' and s[2:4].isnumeric() and s[4] == ' ' and (s[5:7] == 'AM' or s[5:7] == 'PM') and len(s) == 7
    else:
        labubu = False
    return labubu
