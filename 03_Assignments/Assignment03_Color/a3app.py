...
You are welcome to read the code in this file. I guarantee that you will not 
understand it. The TAs do not understand it. That does not matter. When you 
code with another person, you do not need to understand the code they write. 
You just have to write your code to specification.

This is an example of a fairly sophisticated Kivy application. Kivy applications 
are broken into a code file (this) and a layout file (colormodel.kv). The layout 
file serves much the same role as CSS in web pages. In order for it to work it 
must be in the same folder as colormodel.kv. Do not move or change the name of 
that file.
...
class ColorArc(InstructionGroup):
    ...
    def change_value(self, value):
        ...
        The value is external to the segment, and is 1 by default, but can be 
        changed.
...
class HSVWheel(Widget):
    ...
    We have removed alpha, as it is not relevant. We have also removed the 
    gestures, in order to customize this for traditional mouse support. The 
    value property is now external and provided by other input sources.
    """
    ...
    def setSat(self,sat):
        ...
        This method is for "pushing down" a saturation to synchronize it with 
        other input sources.  Direct write access to the huesat attribute is 
        unsafe.
...
class SliderField(BoxLayout):
    ...
    combinations. It keeps the text field and slider in sync and provides a 
    unified observable value for higher-level widgets.
    """
    ...
    def validate_number(self,value):
        ...
        This method uses self.decimals to determine the number of places to 
        round to. If the number is outside of range (min_val to max_val), this 
    ...
    def update_on_slide(self, instance, value):
        ...
        if self.value == value:  # Hack because of poor Kivy design
            maxim = self.max_val/self.factor
            self.value = value+1 if value < maxim else value-1
        self.value = value
        if self.field:
    ...
    def filter_text(self, text, from_undo):
        ...
        This method allows the user to type just enough into the text input 
        field to be in range and have the given number of decimals. Any more 
        than that, and it will stop accepting input.
...
class RGBInputPanel(BoxLayout):
    ...
    This panels stores its internal state as a color property. It consists of 
    three separate slider fields.
    """
...
class HSVInputPanel(BoxLayout):
    ...
    This panels stores its internal state as a color property. It consists of a 
    color wheel, a slider, and three text fields.
    """
    # The current active color (other widgets should observe this)
    # Direct access is only safe for READ-ONLY access
...
class ContrastPanel(BoxLayout):
    ...
    The contrast setting is a localized input. Therefore, we attach it to the 
    color panel that uses it.
    """
    # The font color
    foreground = ListProperty([1,0,0,1])
    # The panel color
...
class ColorWidget(BoxLayout):
    ...
    This class synchronizes the input panels and uses them to set the color 
    settings in the various color panels. It depends heavily on the completion 
    of a3.py. Until that file is completed, it will not do much.
    """
    ...
    def syncInput(self, instance, value):
        ...
        This is called whenever the user updates and input panel. This code uses 
        the conversion functions in a3.py to automatically update the other two 
        panels.
        ...
        if instance == self.rgbPanel:
            origRGB  = introcs.RGB(*value)
            self.rgb = introcs.RGB(*value)
            self.cmyk = a3.rgb_to_cmyk(self.rgb)
            if origRGB != self.rgb:
                raise RuntimeError('rgb_to_cmyk modified the color object')
            elif not self.cmyk is None:
            ...
            self.hsv  = a3.rgb_to_hsv(self.rgb)
            if origRGB != self.rgb:
                raise RuntimeError('rgb_to_hsv modified the color object')
            elif not self.hsv is None:
            ...
            origCMYK  = introcs.CMYK(*value)
            self.cmyk = introcs.CMYK(*value)
            rgb = a3.cmyk_to_rgb(self.cmyk)
            if origCMYK != self.cmyk:
                raise RuntimeError('cmyk_to_rgb modified the color object')
            ...
            origRGB = introcs.RGB(self.rgb.red,self.rgb.green,self.rgb.blue)
            self.hsv  = a3.rgb_to_hsv(self.rgb)
            if origRGB != self.rgb:
                raise RuntimeError('rgb_to_hsv modified the color object')
            elif not self.hsv is None:
            ...
            origHSV  = introcs.HSV(*value)
            self.hsv = introcs.HSV(*value)
            rgb = a3.hsv_to_rgb(self.hsv)
            if origHSV != self.hsv:
                raise RuntimeError('hsv_to_rgb modified the color object')
            elif not rgb is None:
    ...
    def recolor(self):
        ...
        if self.rgb != origRGB:
            raise RuntimeError('complement_rgb modified the color object')
        if (compRGB is None):
            compRGB = self.rgb
...
if __name__ in ('__android__', '__main__'):
    ...
    Factory.register("ConstrastPanel", ContrastPanel)
